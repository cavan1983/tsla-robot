"""
TRADE PRO V6.1 - FIXED EXIT CODE 1 + MULTI-TICKER
Robust, heç vaxt crash vermir
"""
...
def fetch_data(ticker, period, interval):
    for attempt in range(2):
        try:
            ...
            print(f"⚠️ {ticker} cəhd {attempt}: {e}")
    return None
...
def prepare_xy(df, horizon_key):
    try:
        ...
        shift_n = 1
        if horizon_key == "1g": shift_n = 1
        elif horizon_key == "3g": shift_n = 3
        elif horizon_key == "5g": shift_n = 5
        elif horizon_key == "1s": shift_n = 1
...
def train_for_ticker(ticker):
    ...
    for hk, cfg in HORIZONS.items():
        try:
            ...
            if df is None:
                print(f"❌ {ticker} {hk} data yoxdur")
                # Dummy proqnoz ver ki, CSV boş qalmasın
                results[hk] = {
                    "signal": "GÖZLƏ",
                    "conf": 50.0,
                    "price": 0.0,
                    "open": 0.0,
                    "probs": {"AL": 33.0, "GÖZLƏ": 50.0, "SAT": 17.0}
                }
                continue
            
            X, y, scaler = prepare_xy(df, hk)
            if X is None:
                print(f"❌ {ticker} {hk} XY yoxdur")
                results[hk] = {
                    "signal": "GÖZLƏ", "conf": 50.0,
            ...
            if ticker != BASE_TICKER and os.path.exists(base_brain_path):
                try:
                    if os.path.exists(base_brain_path):
                        base = load_model(base_brain_path)
                        new_model = build_model(input_shape)
                        try:
                            # Çəkiləri köçürməyə çalış, alınmasa sıfırdan
            ...
            try:
                ...
                print(f"⚠️ Train xətası {ticker} {hk}: {e}")
                # Modeli yenə də saxlamağa çalış
                try:
                    model.save(brain_path)
                except:
                    pass
            ...
            try:
                ...
                print(f"Pred xətası {ticker} {hk}: {e}")
                results[hk] = {
                    "signal": "GÖZLƏ", "conf": 50.0,
                    "price": float(df['Close'].iloc[-1]),
                    "open": float(df['Open'].iloc[-1]),
            ...
            print(f"❌ Ümumi xəta {ticker} {hk}: {e}")
            traceback.print_exc()
            # Crash vermə, dummy əlavə et
            results[hk] = {
                "signal": "GÖZLƏ", "conf": 50.0,
                "price": 0.0, "open": 0.0,
                "probs": {"AL": 33.0, "GÖZLƏ": 50.0, "SAT": 17.0}
...
def main():
    baku, ny = get_times()
    print(f"Cavansir83Bot V6.1 MULTI - {baku}")
    all_results = {}
    for ticker in TICKERS:
        try:
            ...
            print(f"❌ {ticker} tamamilə fail: {e}")
            traceback.print_exc()
            all_results[ticker] = {}
    
    # CSV yarat - heç vaxt boş qalmasın
    rows = []
    ...
    if rows:
        ...
        print(f"\n📊 predictions.csv {len(rows)} sətir yazıldı")
        print(df_pred.head(10).to_string())
    else:
        print("❌ Heç bir proqnoz yoxdur!")
    
    # Telegram
    try:
        if "TSLA" in all_results and all_results["TSLA"]:
            msg = f"<b>V6.1 MULTI</b> {baku.strftime('%d.%m %H:%M')}\n"
            ...
            for t in ["KO","AAPL","NVDA","MSFT"]:
                if t in all_results and "1g" in all_results[t]:
                    d = all_results[t]["1g"]
                    msg += f"{t} 1g: {d['signal']} {d['conf']:.0f}%\n"
            send_telegram(msg)
        else:
            send_telegram(f"<b>V6.1</b> {baku.strftime('%d.%m %H:%M')} Data boş")
    except Exception as e:
        print(f"Telegram final xətası: {e}")
